...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select(:name, 'COUNT(orders) AS total_order_count').order(:total_order_count).group(:name)
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select('users.name', 'COUNT(items) AS total_item_count').order('users.name').group('users.name')
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins({orders: :items}).select('users.name AS user_name', 'orders.id AS order_id', 'div(orders.amount, COUNT(order_items.id)) AS avg_item_cost').group('user_name', 'orders.id').order(user_name: :desc, avg_item_cost: :asc)
    # ---------------------------------------------------------------
